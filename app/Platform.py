...
class Platform(ABC):
    def __init__(self, name: str = None, url: str = None):
        assert isinstance(name, str) and isinstance(
            url, str
        ), f"[ERROR]: Invalid type, expected str but got {name}: {type(name)}, {url}: {type(url)} instead"
        self.PLATFORM_NAME = name
        self.URL = url
        self.__ASSET_PATH = (
            f"{config.ROOT}/{config.RESOURCES_DIR_NAME}/{self.PLATFORM_NAME}/"
        )

    def get_problem_no(
        self, problem_no: int, timeout_attempt: int = 5, overwrite: bool = False
    ):
        while timeout_attempt > 0:
            try:
                ...
                raise SystemExit(
                    "ERROR: Bad problem number redirecting to too many places"
                )
    ...
    @abstractmethod
    def handler(self, problem_no: int, overwrite: bool = False) -> DTO.ProblemInfo:
        pass

    def prepare_asset(self, file_name: str):
        assert isinstance(
            file_name, str
        ), f"Invalid type. Filename {file_name} is not of type string."
    ...
    def _download_file(self, url: str, path_to_write: str, overwrite: bool):
        if overwrite or not os.path.isfile(path_to_write):
            ...
            print(
                f"[INFO]: {os.path.abspath(path_to_write)} already exists, skipping download"
            )
            return None
...
class Uhunt(Platform):
    ...
    def handler(self, problem_no: int, overwrite: bool = False) -> DTO.ProblemInfo:
        ...
        self._download_file(UHUNT_PDF_URL, FILE_PDF_PATH, overwrite)
        ...
        TEMP_FILE_CONVERT_TEXT_PATH = (
            f"{config.ROOT}/assets/{self.PLATFORM_NAME}/p{problem_no}.txt"
        )
        ...
        with open(TEMP_FILE_CONVERT_TEXT_PATH, mode="r", encoding="utf8") as text_file:
            problem_details = "\n".join(
                [" ".join(_.split()) for _ in text_file.readlines()]
            )
        ...
        return {"title": str(problem_no), "content": problem_details}
...
class ProjectEuler(Platform):
    ...
    def handler(self, problem_no: int, overwrite: bool = False) -> DTO.ProblemInfo:
        ...
        if problem_info is not None:
            problem_title, problem_content = problem.find("h2"), problem.find(
                class_="problem_content"
            )
            ...
            if problem_asset is not None:
                ...
                self._download_file(self.URL + asset_url, asset_filename, overwrite)
            return {
